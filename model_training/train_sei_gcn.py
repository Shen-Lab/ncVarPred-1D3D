		...
		for section_i in range(section_size):
			...
			if(section_i % validation_per_number_of_section == 0):
				...
				if(valid_loss < best_valid_loss):
					...
					torch.save(model, model_output_path + 'Sei_GCN_' + structure_name + '_' + node_feature_type + '_lr{lr}_l2reg{l2reg}_best.pkl'.format(lr = lr, l2reg = lambda_l2))
					patient_count = 0
					torch.save(model, model_output_path + 'Sei_GCN_' + structure_name + '_' + node_feature_type + '_lr{lr}_l2reg{l2reg}_epoch{epoch}_trainloss{trainloss}_validloss{validloss}.pkl'.format(lr = lr, l2reg = lambda_l2, trainloss = round(train_loss, 5), validloss = round(valid_loss, 5), epoch = round(epoch_i + section_i / section_size, 3)))
				else:
