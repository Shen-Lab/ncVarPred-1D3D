	...
	for epoch_i in range(num_epoch):
		...
		for section_i in range(section_size):
			...
			for train_batch_i, (train_batch_seq_x, train_batch_structure_x, train_batch_y) in enumerate(train_dataloader):
				...
				optimizer.zero_grad()
				out = deepsea_concatenation(train_batch_seq_x, train_batch_structure_x)
				if(loss_type == 'BCE'):
				...
				l1_penalty += deepsea_concatenation.structure_fc1.weight.abs().sum()
				l1_penalty += deepsea_concatenation.structure_fc1.bias.abs().sum()
				l1_penalty += deepsea_concatenation.structure_fc2.weight.abs().sum()
				l1_penalty += deepsea_concatenation.structure_fc2.bias.abs().sum()
				l1_penalty += deepsea_concatenation.structure_fc3.weight.abs().sum()
				l1_penalty += deepsea_concatenation.structure_fc3.bias.abs().sum()
			...
			if(section_i % validation_per_number_of_section == 0):
				...
				for valid_batch_i, (valid_batch_seq_x, valid_batch_structure_x, valid_batch_y) in enumerate(valid_dataloader):
					...
					valid_pred_half = deepsea_concatenation(valid_batch_seq_x, valid_batch_structure_x)
