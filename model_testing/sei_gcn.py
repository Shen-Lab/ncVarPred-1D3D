...
class Sei_concatenation(nn.Module):
    def __init__(self, sequence_length=4096, n_genomic_features=21907):
        ...
        """
        super(Sei_concatenation, self).__init__()
        ...
        self.classifier = nn.Sequential(
            nn.Linear(960 * self._spline_df + 128, n_genomic_features),
            nn.ReLU(inplace=True),
    ...
    def forward(self, x, node_input, adj_input, index_input):
        ...
        structure_output = F.relu(self.gcn2(structure_output, adj_input))
        structure_output = self.drop1(structure_output)
        structure_output = F.relu(self.gcn3(structure_output, adj_input))
        structure_output = torch.matmul(index_input, structure_output)
        concatenated_output = torch.cat((reshape_out, structure_output), dim = 1)
        predict = self.classifier(concatenated_output)
        return predict
