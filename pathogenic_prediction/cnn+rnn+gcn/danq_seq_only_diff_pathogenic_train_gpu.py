...
class pathogenic_prediction_model(nn.Module):
	def __init__(self, DanQ, finetune_pathogenic):
	...
	def forward(self, seq_wt_input, seq_mt_input):
		epigenetic_embedding_wt_output = self.DanQ(seq_wt_input)
		epigenetic_embedding_mt_output = self.DanQ(seq_mt_input)
...
def main():
	...
	for epoch_i in range(num_epoch):
		...
		if(valid_loss < best_valid_loss):
			...
			torch.save(pathogenic_prediction, model_output_path + 'danq_seq_only_diff_pathogenic_fewshotsize{few_shot_size}.pkl'.format(few_shot_size = str(int(few_shot_size))))
			patient_count = 0
			torch.save(pathogenic_prediction, model_output_path + 'danq_seq_only_diff_pathogenic_lr{lr}_fewshotsize{few_shot_size}_l2reg{l2reg}_epoch{epoch}_trainloss{trainloss}_validloss{validloss}.pkl'.format(lr = lr, few_shot_size = str(int(few_shot_size)), l2reg = lambda_l2, trainloss = round(train_loss, 5), validloss = round(valid_loss, 5), epoch = epoch_i))
		else:
