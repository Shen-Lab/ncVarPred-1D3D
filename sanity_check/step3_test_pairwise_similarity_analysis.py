...
def pred_similarity_func(pred1, pred2):
	return cosine_similarity([pred1, pred2])[0, 1], 1 - np.mean(np.abs(pred1 - pred2))
...
def main():
	...
	if_matrix = np.load('../graph_data/if_matrix_' + str(int(resolution)) + '/' + hic_name + 'novc_whole_normalized.npy')
	sota_pred_all = np.load('../model_assessment/model_prediction_selected/sota_testpred.npy')
	seq = []
	start_index = []
	label = []
	sota_pred = []
	...
	sota_pred_cosine_similarity_output = []
	cellline_pred_cosine_similarity_output = []
		...
		for sample_j in np.arange(sample_i + 1, len(start_index)):
			if((if_matrix[start_index[sample_i], start_index[sample_j]]) > 0):
				if((np.sum(label[sample_i]) * np.sum(label[sample_j])) >= 1):
					...
					sota_pred_cosine_similarity_output.append(pred_similarity_func(sota_pred[sample_i], sota_pred[sample_j])[0])
					cellline_pred_cosine_similarity_output.append(pred_similarity_func(cellline_pred[sample_i], cellline_pred[sample_j])[0])
					sota_pred_abs_similarity_output.append(pred_similarity_func(sota_pred[sample_i], sota_pred[sample_j])[1])
					cellline_pred_abs_similarity_output.append(pred_similarity_func(cellline_pred[sample_i], cellline_pred[sample_j])[1])
	...
	sota_pred_cosine_similarity_output = np.array(sota_pred_cosine_similarity_output)
	cellline_pred_cosine_similarity_output = np.array(cellline_pred_cosine_similarity_output)
	...
	np.save(output_path + hic_name + 'sota_pred_cosine_similarity_output.npy', sota_pred_cosine_similarity_output)
	np.save(output_path + hic_name + 'concate_pred_cosine_similarity_output.npy', cellline_pred_cosine_similarity_output)
