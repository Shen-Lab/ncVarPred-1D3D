	...
	for epoch_i in range(num_epoch):
		...
		for section_i in range(section_size):
			...
			for train_batch_i, (train_batch_seq_x, train_batch_structure_matching_index, train_batch_y) in enumerate(train_dataloader):
				...
				optimizer.zero_grad()
				out = deepsea_concatenation(train_batch_seq_x, node_feature, structure_all, train_batch_node_selection_index)
				if(loss_type == 'BCE'):
				...
				l1_penalty += deepsea_concatenation.gcn1.weight.abs().sum()
				l1_penalty += deepsea_concatenation.gcn1.bias.abs().sum()
				l1_penalty += deepsea_concatenation.gcn2.weight.abs().sum()
				l1_penalty += deepsea_concatenation.gcn2.bias.abs().sum()
				l1_penalty += deepsea_concatenation.gcn3.weight.abs().sum()
				l1_penalty += deepsea_concatenation.gcn3.bias.abs().sum()
			...
			if(section_i % validation_per_number_of_section == 0):
				...
				for valid_batch_i, (valid_batch_seq_x, valid_batch_node_selection_index, valid_batch_y) in enumerate(valid_dataloader):
					...
					valid_pred_half = deepsea_concatenation(valid_batch_seq_x, node_feature, structure_all, valid_batch_node_selection_index)
