...
def main():
	...
	for snp_subset_i in snp_subset_list:
		for fold_i in fold_list:
			if((snp_subset_i == snp_subset_list[0]) & (fold_i == fold_list[0])):
				x_diff = np.load('model_prediction/GRASP_' + snp_subset_i + '_' + fold_i + '_' + model_name + '_diff.npy')[:, selected_feature]
				x_fc = np.load('model_prediction/GRASP_' + snp_subset_i + '_' + fold_i + '_' + model_name + '_log_odds_fc.npy')[:, selected_feature]
				x_temp = np.concatenate((x_diff, x_fc), 1)
				x_train = x_temp
				y_train = np.zeros(x_temp.shape[0])
			else:
				x_diff = np.load('model_prediction/GRASP_' + snp_subset_i + '_' + str(fold_list[0]) + '_' + model_name + '_diff.npy')[:, selected_feature]
				x_fc = np.load('model_prediction/GRASP_' + snp_subset_i + '_' + str(fold_list[0]) + '_' + model_name + '_log_odds_fc.npy')[:, selected_feature]
				x_temp = np.concatenate((x_diff, x_fc), 1)
				x_train = np.concatenate((x_train, x_temp))
				y_train = np.concatenate((y_train, np.zeros(x_temp.shape[0])))
	for fold_i in fold_list:
		x_diff = np.load('model_prediction/GRASP_eQTL_' + fold_i + '_' + model_name + '_diff.npy')[:, selected_feature]
		x_fc = np.load('model_prediction/GRASP_eQTL_' + fold_i + '_' + model_name + '_log_odds_fc.npy')[:, selected_feature]
		x_temp = np.concatenate((x_diff, x_fc), 1)
		x_train = np.concatenate((x_train, x_temp))
	...
	x_test_diff = np.load('model_prediction/GRASP_' + snp_subset_list[0] + '_' + test_fold + '_' + model_name + '_diff.npy')[:, selected_feature]
	x_test_fc = np.load('model_prediction/GRASP_' + snp_subset_list[0] + '_' + test_fold + '_' + model_name + '_log_odds_fc.npy')[:, selected_feature]
	...
	for snp_subset_i in snp_subset_list[1:]:
		x_test_diff = np.load('model_prediction/GRASP_' + snp_subset_i + '_' + test_fold + '_' + model_name + '_diff.npy')[:, selected_feature]
		x_test_fc = np.load('model_prediction/GRASP_' + snp_subset_i + '_' + test_fold + '_' + model_name + '_log_odds_fc.npy')[:, selected_feature]
	...
	x_test_diff = np.load('model_prediction/GRASP_eQTL_' + test_fold + '_' + model_name + '_diff.npy')[:, selected_feature]
	x_test_fc = np.load('model_prediction/GRASP_eQTL_' + test_fold + '_' + model_name + '_log_odds_fc.npy')[:, selected_feature]
	x_test_temp = np.concatenate((x_test_diff, x_test_fc), 1)
	x_test = np.concatenate((x_test, x_test_temp))
	...
	np.save('model_prediction/GRASP_' + test_fold + '_' + model_name + '_clf_prediction.npy', pred)
	np.save('model_prediction/GRASP_' + test_fold + '_' + model_name + '_clf_cluster_name.npy', cluster_name)
	...
	np.save('model_prediction/GRASP_' + test_fold + '_' + model_name + '_clf_' + selected_feature_name + '_auroc.npy', auroc_summary)
	np.save('model_prediction/GRASP_' + test_fold + '_' + model_name + '_clf_' + selected_feature_name + '_auprc.npy', auprc_summary)
	np.save('model_prediction/GRASP_' + test_fold + '_' + model_name + '_clf_' + selected_feature_name + '_trainpr' + str(round(np.mean(y_train), 4)) + '_testpr' + str(round(np.mean(y_test), 4)) + '_auroc' + str(round(np.mean(auroc_summary), 5)) + '_auprc' + str(round(np.mean(auprc_summary), 5)) + '.npy', np.zeros(1))
