...
def main():
	...
	valid_dataloader = DataLoader(TensorDataset(valid_seq_x, valid_node_selection_index, valid_y), batch_size = 100, shuffle = False, drop_last=False)
	...
	for subset_i in range(19):
		...
		for section_i in range(section_size):
			...
			test_structure_matching_index_npy = np.load(structure_input_matching_path + test_section_chr[section_index[section_i]] + '_section' + str(int(test_section_index[section_index[section_i]])) + '_matching_index.npy')
			...
			test_dataloader = DataLoader(TensorDataset(test_seq_x, test_node_selection_index, test_y), batch_size = 128, shuffle = False, drop_last=False)		
			for test_batch_i, (test_batch_seq_x, test_batch_node_selection_index, test_batch_y) in enumerate(test_dataloader):
				...
				test_pred_firsthalf = model(test_batch_seq_x, node_feature, structure_all, test_batch_node_selection_index).cpu().detach().numpy()[:, int(subset_i * 1153):int(subset_i * 1153 + 1153)]
				...
				test_batch_seq_x = test_batch_seq_x.cuda()
				test_pred_secondhalf = model(test_batch_seq_x, node_feature, structure_all, test_batch_node_selection_index).cpu().detach().numpy()[:, int(subset_i * 1153):int(subset_i * 1153 + 1153)]
