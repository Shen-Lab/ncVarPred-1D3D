...
def label_similarity_func(label1, label2):
	#modified
	return(np.sum(label1 == label2))

def seq_similarity_func(seq1, seq2):
	#modified
	return 1000 - (np.sum(seq1 != seq2) / 2)
	...
	for sample_i in range(len(start_index) - 1):
		for sample_j in np.arange(sample_i + 1, len(start_index)):
			if((if_matrix[start_index[sample_i], start_index[sample_j]]) > 0):
				seq_similarity_output.append(int(seq_similarity_func(seq[sample_i], seq[sample_j])))
				label_similarity_output.append(int(label_similarity_func(label[sample_i], label[sample_j])))
				if_similarity_output.append(int(if_matrix[start_index[sample_i], start_index[sample_j]] * 10000))
	seq_similarity_output = np.asarray(seq_similarity_output, dtype = 'int16')
	label_similarity_output = np.asarray(label_similarity_output, dtype = 'int16')
	if_similarity_output = np.asarray(if_similarity_output, dtype = 'int16')
