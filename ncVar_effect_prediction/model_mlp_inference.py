...
def main():
	...
	sota_model.eval()
	sota_model.cuda()
	structure_all = np.load(structure_path)
	inference_seq_x = np.swapaxes(np.load(seq_path), 1, 2)
	structure_matching_index = np.load(structure_matching_path)
	inference_structure_x = structure_all[np.asarray(structure_matching_index, dtype = int), :]
	...
	inference_dataloader = DataLoader(TensorDataset(inference_seq_x, inference_structure_x), batch_size = 128, shuffle = False, drop_last=False)
	sample_index = 0
	for inference_batch_i, (inference_batch_seq_x, inference_batch_structure_x) in enumerate(inference_dataloader):
		...
		our_pred_firsthalf = our_model(inference_batch_seq_x, inference_batch_structure_x).cpu().detach().numpy()[:, feature_selected_boolean]
		...
		our_pred_secondhalf = our_model(inference_batch_seq_x, inference_batch_structure_x).cpu().detach().numpy()[:, feature_selected_boolean]
